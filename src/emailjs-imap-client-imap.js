(function (root, factory) {
  ...
  if (typeof define === "function" && define.amd) {
    define([
      ...
      "emailjs-mime-codec",
      "./emailjs-imap-client-compression",
    ], factory);
  } else if (typeof exports === "object") {
    module.exports = factory(
      require("emailjs-tcp-socket"),
      require("emailjs-imap-handler"),
      require("emailjs-mime-codec"),
      require("./emailjs-imap-client-compression"),
      null
    );
  }
  ...
  function Imap(host, port, options) {
    ...
    this.options.useSecureTransport =
      "useSecureTransport" in this.options ? !!this.options.useSecureTransport : this.port === 993;
      ...
      var data = {
        ...
        callback: (response) => {
          if (this.isError(response)) {
            ...
            ["NO", "BAD"].indexOf(((response && response.command) || "").toString().toUpperCase().trim()) >= 0
          ) {
  ...
  Imap.prototype.send = function (str) {
    ...
    this._socketTimeoutTimer = setTimeout(
      () => this._onError(new Error(this.options.sessionId + " Socket timed out!")),
      timeout
  ...
  Imap.prototype.enableCompression = function () {
    ...
    if (typeof window !== "undefined" && window.Worker && typeof this._workerPath === "string") {
      ...
      this._compressionWorker.onerror = (e) => {
        this._onError(
          new Error("Error handling compression web worker: Line " + e.lineno + " in " + e.filename + ": " + e.message)
        );
      };
